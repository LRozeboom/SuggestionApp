...
public class MongoSuggestionData : ISuggestionData
{
    ...
    public async Task<List<SuggestionModel>> GetAllSuggestions()
    {
        ...
        if (output is null)
        {
            ...
            _cache.Set(CacheName, TimeSpan.FromMinutes(1));
        }
    ...
    public async Task UpvoteSuggestion(string suggestionId, string userId)
    {
        ...
        try
        {
            ...
            // As HashSet can only have one entry for each key, false is returned if it is already upvoted -> hence it is a downvote
